...
class DQN_method:
    ...
    def build_model(self):
        ...
        self.q_target = tf.placeholder(tf.float32, [None, self.action_dim], name='Q_target')  # for calculating
        ...
        with tf.variable_scope('eval_network'):
            ...
            with tf.variable_scope('l2'):
                w2 = tf.get_variable('w2', [n_l1, self.action_dim], initializer=w_initializer, collections=c_names)
                b2 = tf.get_variable('b2', [1, self.action_dim], initializer=b_initializer, collections=c_names)
    ...
    def Learn(self):
        ...
        q_target = self.sess.run(self.q_eval, feed_dict={self.s: batch_state})
        q_next = self.sess.run(self.q_next, feed_dict={self.s_: batch_state_next})
        q_next_Max = np.max(q_next, axis=1)
        batch_index = np.arange(self.batch_size, dtype=np.int32)
        act_index = batch_action.astype(int)
        q_target[batch_index, act_index] = batch_reward + self.gamma * q_next_Max
